...
Chrome is controlled by Selenium Manager. If the workstation is offline, put a compatible driver path in `chromedriver_path`.

Each run log records `Browser mode: Background (headless)` or `Browser mode: Visible windows` near the top. Every subsequent Chrome readiness entry repeats the active mode so the UI selection can be verified for each browser session.
...
IRT validation is fail-closed. Before downloading order PDFs, the collector runs a 14-day current-inventory availability check for `STMIAP00`; an unavailable, stalled, or confirmed-empty IRT search stops the run before the expensive download and OCR phase. IRT's visible `Please wait` blocker is the primary completion signal: while it remains visible, the collector ignores the temporary `No data found` row and any transitional record count. Once the blocker disappears, the table must settle before its final rows or confirmed empty result are accepted. The wait remains Stop-responsive and uses `irt_overlay_timeout_seconds` (600 seconds by default) only as an emergency ceiling for unusually large searches. All PDFs are then downloaded and renamed in temporary storage first. The collector searches from the oldest certified decision date found through the end of the operator's range, captures every available results page, validates the displayed total against the number captured, and compares the renamed files against that local index. A timed-out, suspiciously empty, stale, or incomplete snapshot restarts Chrome and retries in fresh sessions. This catches an older certified order that the court site released or reposted during the selected window. A confirmed duplicate is deleted; a confirmed non-duplicate is finalized; and exhausting every fresh-Chrome IRT attempt prevents every unverified PDF from being finalized.
...
Order filenames use the certified decision date read from the rendered footer. The collector OCRs the lower portion of each order's final pages and anchors the date to the certification/signature block. A certified footer date can override a newer live-site release date for a reposted order; dates found only in the body remain unusable. When the certification block is visibly present but its Date field is blank, the untouched source PDF is saved in the run's `Excluded` folder and reported as `missing_certified_date`. Other failures to establish a trustworthy certified date remain errors and fail closed instead of allowing a deadline, hearing date, or website release date into the filename.
